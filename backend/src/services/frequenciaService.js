...
async function listarFrequencia(query) {
  ...
  const servidorId = normalizeText(query?.servidorId);
  const servidorCpf = onlyDigits(query?.servidorCpf);
...
async function criarOcorrencia(body) {
  const payload = {
    servidor_id: normalizeText(body?.servidorId || body?.servidor_id),
    servidor_cpf: onlyDigits(body?.servidorCpf || body?.servidor_cpf),
...
async function atualizarOcorrencia(id, body) {
  ...
  if (body?.descricao !== undefined) {
    payload.descricao = normalizeText(body?.descricao);
  }
