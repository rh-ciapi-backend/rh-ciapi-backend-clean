...
function detectLayout(rawRow, requestedLayout = 'automatico') {
  if (requestedLayout && requestedLayout !== 'automatico') {
  ...
  if (
    categoria.includes('SESAU') &&
    (
      rawRow.data_inicio_setor ||
      rawRow.dataInicioSetor ||
      rawRow.carga_horaria ||
      rawRow.cargaHoraria ||
      rawRow.lotacao_interna ||
      rawRow.lotacaoInterna
    )
  ) {
  ...
  if (
    categoria.includes('SESAU') &&
    (rawRow.matricula_sigrh || rawRow.matriculaSigrh || rawRow.sigrh)
  ) {
...
function normalizeLinhaMapa(rawRow, index = 0, requestedLayout = 'automatico') {
  ...
  const matricula = text(
    rawRow.matricula ||
    rawRow.matricula_servidor ||
    rawRow.servidor_matricula
  );
  ...
  if (!text(rawRow.nome_completo || rawRow.nome || rawRow.servidor_nome)) {
    inconsistencias.push('Nome ausente');
  }
  ...
  if (!text(rawRow.setor || rawRow.lotacao)) inconsistencias.push('Setor ausente');

  if (
    tipoLayout === 'sesau_detalhado' &&
    !text(rawRow.data_inicio_setor || rawRow.dataInicioSetor)
  ) {
    inconsistencias.push('Data início no setor ausente');
  }

  if (
    tipoLayout === 'sesau_detalhado' &&
    !text(rawRow.carga_horaria || rawRow.cargaHoraria)
  ) {
    inconsistencias.push('Carga horária ausente');
  }

  if (
    tipoLayout === 'sesau_seletivo' &&
    !text(rawRow.matricula_sigrh || rawRow.matriculaSigrh || rawRow.sigrh)
  ) {
    inconsistencias.push('Matrícula SIGRH ausente');
  }

  return {
    ordem: Number(rawRow.ordem || index + 1),
    matricula,
    matriculaSigrh: text(rawRow.matricula_sigrh || rawRow.matriculaSigrh || rawRow.sigrh),
    ...
    setor: text(rawRow.setor || rawRow.lotacao, 'NÃO INFORMADO'),
    lotacao: text(rawRow.lotacao || rawRow.setor, ''),
    lotacaoInterna: text(rawRow.lotacao_interna || rawRow.lotacaoInterna || rawRow.setor),
